...
let parse_deps cctx ~file ~unit ~(stdlib:Dune_file.Library.Stdlib.t option)
      lines =
...
let deps_of cctx ~stdlib ~ml_kind unit =
    ...
    | Some file ->
      ...
      SC.add_rule sctx
        ( Build.lines_of ocamldep_output
          >>^ parse_deps cctx ~file ~unit ~stdlib
          >>^ (fun modules ->
            (build_paths modules,
...
let rules_generic cctx ~stdlib ~modules =
  Ml_kind.Dict.of_func
    (fun ~ml_kind ->
       let per_module =
         Module.Name.Map.map modules ~f:(deps_of cctx ~stdlib ~ml_kind)
       in
       { Dep_graph.
         dir = CC.dir cctx
       ; per_module
...
let rules ?stdlib cctx = rules_generic cctx ~stdlib ~modules:(CC.modules cctx)

let rules_for_auxiliary_module cctx (m : Module.t) =
  rules_generic cctx ~modules:(Module.Name.Map.singleton m.name m) ~stdlib:None
