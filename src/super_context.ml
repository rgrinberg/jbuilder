...
let add_rule t ?sandbox ?mode ?locks ?loc ?backtrace ~dir build =
  let build = Build.O.(>>>) build t.chdir in
  let env = Env.external_ t ~dir in
  Build_system.add_rule t.build_system
    (Build_interpret.Rule.make ?sandbox ?mode ?locks ?loc ?backtrace
       ~context:(Some t.context) ~env:(Some env) build)

let add_rule_get_targets t ?sandbox ?mode ?locks ?loc ?backtrace ~dir build =
  let build = Build.O.(>>>) build t.chdir in
  let env = Env.external_ t ~dir in
  let rule =
    Build_interpret.Rule.make ?sandbox ?mode ?locks ?loc ?backtrace
...
let add_rules t ?sandbox ?backtrace ~dir builds =
  List.iter builds ~f:(add_rule t ?sandbox ?backtrace ~dir)
