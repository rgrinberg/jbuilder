...
let make ~ocaml_config ~pforms ~pkg_version ~artifacts ~host_artifacts =
  { ocaml_config
  ; pforms
  ; pkg_version
  ; artifacts
...
let add_bindings t bindings =
  { t with pforms = Pform.Map.superpose t.pforms bindings }

let bindings t = t.pforms
...
module Dynamic = struct
  ...
  let expander ~acc t ~dir ~dep_kind ~scope ~targets_written_by_user
        ~map_exe pform syntax_version =
    let loc = String_with_vars.Var.loc pform in
    let key = String_with_vars.Var.full_name pform in
    let res =
      Pform.Map.expand t.pforms pform syntax_version
      |> Option.bind ~f:(function
        | Pform.Expansion.Var (Values l) -> Some l
        | Macro (Ocaml_config, s) ->
          Some (Static.expand_ocaml_config t pform s)
        ...
        | Macro (Bin, s) -> begin
            match Artifacts.binary ~loc:None t.host_artifacts s with
        ...
        | Macro (Libexec, s) -> begin
            ...
            Resolved_forms.add_lib_dep acc lib_dep dep_kind;
            match
              Artifacts.file_of_lib t.host_artifacts ~loc ~lib:lib_dep ~file
            with
            | Error fail -> Resolved_forms.add_fail acc fail
            | Ok path ->
        ...
        | Macro (Version, s) -> begin
            ...
            | Some p ->
              let x =
                Pkg_version.read t.pkg_version p >>^ function
                | None   -> [Value.String ""]
                | Some s -> [String s]
              in
  ...
  let with_expander t ~dir ~dep_kind ~scope ~targets_written_by_user
        ~map_exe ~f =
    let acc = Resolved_forms.empty () in
    ( f (expander ~acc t ~dir ~dep_kind ~scope
