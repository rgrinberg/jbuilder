...
module File = struct
  ...
  module Stanza = struct
    ...
    (** Defines uniqueness criteria for stanzass *)
    module Signature = struct
      ...
      let equal a b =
        ...
        (match a.public_name, b.public_name with
         | None, _| _, None -> false
         | Some a, Some b   -> String.equal a b)
    end
    ...
    let to_string s =
      ...
      | None -> ""
      | Some s ->  Dune_lang.(to_string ~syntax:Dune s)
    ...
    let find_conflicting new_stanzas existing_stanzas =
      let stanzas_conflict a b =
        let open Option.O in
        Option.value ~default:false
          (Signature.of_cst a >>= fun a ->
           Signature.of_cst b >>| fun b ->
           Signature.equal a b)
      in
      let conflicting_stanza stanza =
        ...
        | None -> None
      in
      List.map ~f:conflicting_stanza new_stanzas
  ...
  let create_dir path =
    ...
    | Unix.Unix_error (EACCES, _, _) ->
      Exn.fatalf
        "A project directory cannot be created or accessed: \
  ...
  let load_dune_file ~path =
    ...
    let content =
      if not (Path.exists full_path) then [] else
...
module Add_context = struct
  ...
  let make root_str path =
    let root = Path.of_string root_str in
    let project =
      match Dune_project.load ~dir:root ~files:String.Set.empty with
      | Some p -> p
...
module Component = struct
  ...
  module Stanza_cst = struct
    ...
    let executable common (options : Options.executable) =
      let public_name =
        public_name_field ~default:Options.(common.name) options.public
      in
      make "executable" Options.({common with name = "main"}) public_name

    let library common (options: Options.library) =
      let common, inline_tests =
        if not options.inline_tests then
          common, []
        else
          let pps =
            add_to_list_set "ppx_inline_tests" Options.(common.pps)
          in
          Options.{common with pps}, [Field.inline_tests]
      in
      let public_name =
        public_name_field ~default:Options.(common.name) options.public
      in
  ...
  let test ({context; common; options}: Options.test Options.t) =
    ...
    in
    let test_ml =
      let name = Printf.sprintf "%s.ml" common.name in
      let content = "" in
      File.make_text dir name content
    in
...
let validate_component_name name =
  ...
  | Ok _ -> ()
  | _ -> Exn.fatalf "A component named '%s' cannot be created because it is an %s"
