      ...
      if not lib.wrapped ||
        ...
        Some
          (Module.make main_module_name
             ...
             ~obj_name:lib.name)
    in
    { modules; alias_module; main_module_name ; virtual_modules }
end
...
let build_modules_map (d : Dir_with_jbuild.t) ~modules =
  let libs, exes =
...
let build_mlds_map (d : Dir_with_jbuild.t) ~files =
...
module Dir_status = struct
  type t =
    | Standalone of (File_tree.Dir.t * Dir_with_jbuild.t option) option
    ...
    | Group_root of File_tree.Dir.t * Dir_with_jbuild.t
    (* Directory with [(include_subdirs x)] where [x] is not [no] *)

    | Is_component_of_a_group_but_not_the_root of Dir_with_jbuild.t option
