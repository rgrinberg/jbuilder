...
let build_modules_map (d : _ Dir_with_dune.t) ~modules =
  ...
  let libs, exes =
    List.filter_partition_map d.data ~f:(fun stanza ->
      ...
      | Library lib ->
        let obj_dir =
          Module.Obj_dir.make_local ~dir:d.ctx_dir (snd lib.name)
            ~has_private_modules:(lib.private_modules <> None)
        in
      ...
      | Tests { exes; _} ->
        let obj_dir =
          Module.Obj_dir.make_exe ~dir:d.ctx_dir (List.hd exes.names |> snd)
        in
        let { Modules_field_evaluator.
              all_modules = modules
            ; virtual_modules = _
