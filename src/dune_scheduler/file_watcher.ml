    ...
    List.map events ~f:(fun event ->
      Dune_trace.Event.file_watcher
        (match event with
         ...
         | Filesystem_event Watcher_terminated -> `Watcher_terminated
...
module Fs_sync : sig
  ...
  val emit : Sync.Table.t -> unit Fiber.Ivar.t -> unit
  ...
  let emit sync_table ivar =
    let id = Sync.Id.gen () in
    let sync = { Sync.id; ivar } in
    let fn = Sync.Id.to_int id |> string_of_int in
    let path = Filename.concat (Lazy.force special_dir) fn in
    Sync.Table.create_sync_file sync_table ~fn ~sync ~path
  ;;
  ...
  let (_ : Thread.t) =
    Thread0.spawn ~name:"file-watcher" (fun () ->
      ...
      let rec loop () =
        ...
        | exception End_of_file -> enqueue [ Filesystem_event Event.Watcher_terminated ]
        | path_s ->
          let events =
...
let%expect_test "fsevents standard event handling" =
  let print
    ...
    | Some (Sync _ | Filesystem_event Event.Watcher_terminated) ->
      ...
      | dispatch_queue ->
        ...
        (match Fsevents.Dispatch_queue.wait_until_stopped dispatch_queue with
         ...
         | Error exn ->
           ...
           dispatch_backend_events
             event_channel
             [ Filesystem_event Event.Watcher_terminated ]))
  in
  let dispatch_queue =
    match
      Mutex.protect mutex (fun () ->
...
let fswatch_win_event ~sync_table ~should_exclude event =
  ...
  | In_build_dir _ ->
    ...
    then (
      ...
      | Added | Modified ->
        Option.map (Sync.Table.consume_event sync_table filename) ~f:(fun sync ->
          Sync sync)
...
let create ?fsevents_debounce ~watch_exclusions ~events () =
  ...
  let kind =
    ...
    | `Fswatch _ as backend ->
      create_external_fswatch ~sync_table ~event_channel:events ~backend ~watch_exclusions
    | `Fsevents ->
      create_fsevents
        ~sync_table
        ...
        ~should_exclude
        ()
    | `Inotify_lib -> create_inotifylib ~sync_table ~event_channel:events ~should_exclude
    | `Fswatch_win ->
      create_fswatch_win
        ~sync_table
        ~event_channel:events
...
let flush t =
  ...
  Fs_sync.emit t.sync_table ivar;
