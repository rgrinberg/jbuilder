...
/* The signal handler must never depend on GC-managed wakeup resources. Keep
   one process-global wakeup object and intentionally let the OS reclaim it at
   process exit. */
static PVOID volatile signal_wakeup = NULL;
static HANDLE global_event_wakeup = NULL;

static HANDLE get_global_event_wakeup(void)
{
  if (global_event_wakeup == NULL) {
    HANDLE event = CreateEvent(NULL, FALSE, FALSE, NULL);
    if (event == NULL) caml_failwith("CreateEvent");
    global_event_wakeup = event;
  }
  return global_event_wakeup;
}

static void reset_global_event_wakeup(void)
{
  HANDLE event = get_global_event_wakeup();
  DWORD result;
  ...
  if (result != WAIT_TIMEOUT) caml_failwith("WaitForSingleObject");
...
CAMLprim value dune_event_wakeup_create(value v_unit)
{
  ...
  (void)v_unit;
  reset_global_event_wakeup();
  v_wakeup = caml_alloc_custom(&dune_event_wakeup_ops,
                               sizeof(struct dune_event_wakeup), 0, 1);
  ((struct dune_event_wakeup *)Data_custom_val(v_wakeup))->unused = 0;
...
CAMLprim value dune_event_wakeup_release(value v_wakeup)
{
  CAMLparam1(v_wakeup);
  (void)v_wakeup;
  if (!SetEvent(get_global_event_wakeup())) caml_failwith("SetEvent");
  CAMLreturn(Val_unit);
}
...
CAMLprim value dune_event_wakeup_try_acquire(value v_wakeup)
{
  ...
  (void)v_wakeup;
  result = WaitForSingleObject(get_global_event_wakeup(), 0);
...
CAMLprim value dune_event_wakeup_use_for_signals(value v_wakeup)
{
  ...
  InterlockedExchangePointer(&signal_wakeup, get_global_event_wakeup());
...
CAMLprim value dune_event_wakeup_stop_using_signals(value v_wakeup)
{
  CAMLparam1(v_wakeup);
  HANDLE event = get_global_event_wakeup();
  PVOID previous = InterlockedCompareExchangePointer(&signal_wakeup, NULL, event);
...
/* The signal handler must never depend on GC-managed wakeup resources. Keep
   one process-global wakeup object and intentionally let the OS reclaim it at
   process exit. */
static sem_t *global_event_wakeup = NULL;
...
static sem_t *get_global_event_wakeup(void)
{
  if (global_event_wakeup == NULL) global_event_wakeup = create_semaphore();
  return global_event_wakeup;
}
...
static void reset_global_event_wakeup(void)
{
  sem_t *sem = get_global_event_wakeup();

  while (try_wait_for_semaphore(sem) == 0) {}
  if (errno != EAGAIN) uerror("sem_trywait", Nothing);
  signal_wakeup_clear_pending();
}
...
CAMLprim value dune_event_wakeup_release(value v_wakeup)
{
  CAMLparam1(v_wakeup);
  (void)v_wakeup;
  if (sem_post(get_global_event_wakeup()) == -1) uerror("sem_post", Nothing);
  CAMLreturn(Val_unit);
}
...
CAMLprim value dune_event_wakeup_try_acquire(value v_wakeup)
{
  CAMLparam1(v_wakeup);
  (void)v_wakeup;
  if (try_wait_for_semaphore(get_global_event_wakeup()) == 0) {
    signal_wakeup_clear_pending();
    CAMLreturn(Val_true);
  }
...
CAMLprim value dune_event_wakeup_use_for_signals(value v_wakeup)
{
  ...
  register_signal_wakeup_root();
  signal_wakeup_root = v_wakeup;
  signal_wakeup_set(get_global_event_wakeup());
  /* A previous scheduler run may have left a stale wakeup token on its
     semaphore. */
  signal_wakeup_clear_pending();
  CAMLreturn(Val_unit);
...
{
  CAMLparam1(v_wakeup);
  if (signal_wakeup_unset_if(get_global_event_wakeup())) {
    wait_for_signal_handlers();
    signal_wakeup_clear_pending();
    signal_wakeup_root = Val_unit;
  }
