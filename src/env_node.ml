...
type t =
  ...
  ; mutable c_flags       : (unit, string list) Build.t C.Kind.Map.t option
...
let rec c_flags t ~profile ~expander ~default_context_flags =
  ...
  | None ->
    let default =
      match t.inherit_from with
      | None -> C.Kind.Map.map ~f:Build.return default_context_flags
    ...
    let flags =
      ...
      | Some cfg ->
        let expander = Expander.set_dir expander ~dir:t.dir in
        C.Kind.Map.mapi cfg.c_flags ~f:(fun ~kind f ->
