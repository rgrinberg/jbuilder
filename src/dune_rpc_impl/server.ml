...
let handler (t : t Fdecl.t) : 'a Dune_rpc_server.Handler.t =
  ...
  let () =
    ...
    let module Error = Build_config.Error in
    let equal (s : Error.Set.t) (t : Error.Set.t) = Int.equal s.stamp t.stamp in
    let diff ~last ~(now : Error.Set.t) =
      match last with
      | None ->
        Error.Id.Map.to_list_map now.current ~f:(fun _ e ->
            Diagnostic.Event.Add (Diagnostics.diagnostic_of_error e))
      | Some (prev : Error.Set.t) -> (
        match prev.last_event with
        | Some last_event when prev.stamp + 1 = now.stamp ->
        ...
        | _ ->
          ...
          Error.Id.Map.merge prev.current now.current ~f:(fun _ prev now ->
              match (prev, now) with
    ...
    Handler.implement_long_poll rpc Procedures.Poll.diagnostic
      Build_system.errors ~equal ~diff
  in
  let () =
    let diff ~last:_ ~(now : Build_system.State.t) =
      ...
      Dune_rpc.Progress.In_progress { complete; remaining }
    in
  ...
  let () =
    let f _ () =
      ...
      errors.current |> Build_config.Error.Id.Map.values
