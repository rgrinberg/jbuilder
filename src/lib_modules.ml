...
let make_alias_module ~(obj_dir:Module.Obj_dir.t) ~implements ~lib_name ~stdlib
...
let version_installed ~install_dir:(dir) t =
  let obj_dir = Module.Obj_dir.make_external ~dir in
