...
let setup_ml_deps ~scope ~stdlib_dir ~loc libs =
  ...
  let ml_iflags, mlpack =
    ...
    | Ok libs ->
      Util.include_flags ~stdlib_dir libs, List.concat_map ~f:ml_pack_files libs
    | Error exn ->
...
let setup_rules ~sctx ~dir ~dir_contents (s : Dune_file.Coq.t) =
  ...
  let ml_iflags, mlpack_rule =
    setup_ml_deps ~scope ~stdlib_dir ~loc:s.loc s.libraries in
