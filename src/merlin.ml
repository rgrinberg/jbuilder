...
module Preprocess = struct
  ...
  let merge a b =
    ...
    | Pps _, Other -> a
    | Other, Pps _ -> b
    | Pps { loc = _; pps = pps1; flags = flags1; using_typer = ut1 },
      Pps { loc = _; pps = pps2; flags = flags2; using_typer = ut2 } ->
      match
        match Bool.compare ut1 ut2 with
        | Gt| Lt as ne -> ne
        | Eq ->
...
let ppx_flags sctx ~dir:_ ~scope ~dir_kind { preprocess; libname; _ } =
  ...
  | Pps { loc = _; pps; flags; using_typer = _ } -> begin
