...
let request_restart t invalidation =
  ...
  then Fiber.return ()
    ...
    let now = Time.now () in
    let input_change_run_id = Run_id.Watch !next_run_id in
...
let run_current_build t ~run_id step =
  ...
  let* outcome =
    Scheduler.with_current_build_cancellation cancellation (fun () ->
  ...
  let next =
    ...
    | Standing_by | Building ->
      ...
      `Done
  in
  Fiber.return (outcome, next, build_start_input_change_run_id)
;;
