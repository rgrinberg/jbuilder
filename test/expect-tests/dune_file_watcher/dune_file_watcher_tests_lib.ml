...
let create_watcher ?fsevents_debounce ~watch_exclusions () =
  ...
  let handle_events events =
    List.filter_map events ~f:(function
      | Dune_scheduler.Event.File_watcher_event.Fs_memo_event event -> Some event
  ...
  let events = Dune_scheduler.Thread_safe_channel.create event_queue in
  let watcher =
    Dune_scheduler.File_watcher.create ?fsevents_debounce ~watch_exclusions ~events ()
  in
  let rec read_events () =
    let open Fiber.O in
